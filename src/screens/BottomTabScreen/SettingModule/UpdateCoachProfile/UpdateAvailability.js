...
const UpdateAvailability = ({ navigation }) => {
    ...
    const toggleDay = (day) => {
        ...
        setAvailability((prevAvailability) => {
            ...
            updatedAvailability[day] = {
                ...updatedAvailability[day],
                enabled: !updatedAvailability[day].enabled,
            };
            return updatedAvailability;
        });
    };
    ...
    const addAvailability = (day, startTime, endTime) => {
        setAvailability(prevAvailability => {
            ...
            const newTimeSessions = [
                // ...dayAvailability.timeSessions,
                { start: moment(startTime).format('hh:mm'), end: moment(endTime).format('hh:mm') },
                ...dayAvailability.timeSessions
            ];
            return {
                ...updatedAvailability,
    ...
    const removeTimeSession = (day, index) => {
        setAvailability(prevAvailability => {
            const updatedAvailability = { ...prevAvailability };
            const dayAvailability = { ...updatedAvailability[day] };
            const newTimeSessions = [...dayAvailability.timeSessions];

            // Check if index is valid before removing
            if (index >= 0 && index < newTimeSessions.length) {
                newTimeSessions.splice(index, 1);
            }

            // Check if timeSessions array is empty after removal
            const enabled = newTimeSessions.length > 0;
    ...
    const renderDepartureSheet = () => {
        return <BottomSheet modalProps={{}} isVisible={departureSheetVisible}>
            <View
                style={{
                    backgroundColor: '#fff',
                    width: "100%",
            ...
            >
                ...
                <CustomButton
                    onPress={() => {
                        ...
                        addAvailability(selectedDay, date, endDate);
                    }}
                    title={'Save Time'}
                    customStyle={{
                        marginTop: 10,
    ...
    const removeSeconds = (time) => {
        ...
        if (timeParts.length > 2) {
            ...
            return time; // No seconds present, return unchanged time string
        }
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <Text style={{
                ...
                textAlign: 'center',
                marginTop: 30,
            }}>Set the hours you are free to accept sessions</Text>
                                ...
                                <View key={index} style={{
                                    ...
                                    <View key={index} style={{
                                        ...
                                        <Text style={{
                                            ...
                                            margin: 8,
                                            fontFamily: fonts.fontsType.regular
                                        }}>{`${removeSeconds(timeSession.start) + 'am-' + removeSeconds(timeSession.end)}pm`}</Text>
